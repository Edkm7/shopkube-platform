...
Note : TLS est désactivé sur Vault lui-même car il tourne sur un réseau interne. En production publique, 
Vault devrait être derrière un reverse proxy avec TLS
...
Note production : en environnement de production, les clés de déchiffrement Vault sont stockées dans un boîtier HSM
 (Hardware Security Module) qui effectue les opérations cryptographiques sans jamais exposer les clés en clair. Des 
recovery keys sont générées séparément et conservées hors ligne en cas de perte ou de défaillance du HSM. Dans ce lab, les clés sont stockées manuellement de façon sécurisée à titre d'apprentissage uniquement.
...
vault operator unseal  # répéter 3 fois avec 3 clés différentes
vault status           # Sealed: false confirme le déverrouillage
...
Important : vérifier la durée de la CA après création. Si le TTL n'a pas été pris en compte, la CA peut avoir une durée 
par défaut bien inférieure à 10 ans.
